...
def diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource=False, augment=False):
...
@torch.no_grad()
def text2image_ldm_stable_ratio(
    model,
    prompt: List[str],
    controller,
    num_inference_steps: int = 50,
    ...
    ratio_steps = int(num_inference_steps * ratio)
    for t in tqdm(model.scheduler.timesteps[:ratio_steps]):
        latents = diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource)
    
    return latents, text_embeddings, uncond_embeddings
...
def text2image_ldm_stable(
    ...
    model.scheduler.set_timesteps(num_inference_steps, **extra_set_kwargs)
    for t in tqdm(model.scheduler.timesteps):
        latents = diffusion_step(model, controller, latents, context, t, guidance_scale, low_resource, augment=True)
...
def test_get_time_words_attention_alpha():
    return
